...
public:
    ...
    OthelloBoard(std::array<std::array<int, SIZE>, SIZE> b){
...
void write_valid_spot(std::ofstream& fout) {
    ...
    for(int i=0;i<n_valid_spots;i++){
//set
    OthelloBoard curboard=OthelloBoard(board);
    dep=curboard.disc_count[EMPTY];
    alphabeta(curboard,curboard.disc_count[EMPTY],-10000000,10000000,player);
...
int alphabeta(OthelloBoard board,int depth,int alpha,int beta,int pl){
    if(depth==0||board.disc_count[BLACK]==0||board.disc_count[WHITE]==0){
        return heuristic(board);
    }
    if(pl==player){
        if(board.next_valid_spots.size()!=0){
            int value=-10000000;
            for(int i=0;i<board.next_valid_spots.size();i++){
                OthelloBoard tb=board;
                tb.put_disc(board.next_valid_spots[i]);
                int tv=alphabeta(tb,tb.disc_count[EMPTY],alpha,beta,tb.cur_player);
                if(depth==dep){
                    if(value<tv) value=tv,pp=board.next_valid_spots[i];
                }
                else{
                    //is it the best??
                    value=max(value,tv);
        ...
        if(board.next_valid_spots.size()!=0){
            ...
            for(int i=0;i<board.next_valid_spots.size();i++){
                OthelloBoard tb=board;
                tb.put_disc(board.next_valid_spots[i]);
                value=min(value,alphabeta(tb,tb.disc_count[EMPTY],alpha,beta,tb.cur_player));
                beta=min(value,beta);
                if(beta<=alpha)break;
            }
            return value;
...
int heuristic(OthelloBoard board){
    return board.disc_count[player];
}

int main(int curplay, char** argv) {
    std::ifstream fin(argv[1]);
    std::ofstream fout(argv[2]);
    read_board(fin);
    read_valid_spots(fin);
    write_valid_spot(fout,curplay);
    fin.close();
    fout.close();
    return 0;
}
